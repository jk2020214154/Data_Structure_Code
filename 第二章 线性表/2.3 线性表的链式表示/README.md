...
>
>  ![](https://cdn.acwing.com/media/article/image/2023/07/22/85276_89a51b8028-20230722102827.png) 
>
...
>  **2015统考真题**：用单链表保存 $m$个整数，结点的结构为$ [data] [link]$，且 $\left| data \right| \leq n$ ( $n$为正整数)。现要求设计一个时间复杂度尽可能高效的算法，对于链表中 $data$的绝对值相等的结点，仅保留第一次出现的结点而删除其余绝对值相等的结点。例如，若给定的单链表 $head$如下:
>
> ![](https://cdn.acwing.com/media/article/image/2023/07/22/85276_fc90c2e328-20230722105953.png)
>
> 则删除结点后的 $head$为
>
>  ![](https://cdn.acwing.com/media/article/image/2023/07/22/85276_18a157a728-20230722110011.png)
>
> 要求 :
>
> 1)给出算法的基本设计思想。
...
用一个计数数组保存 $\left| data \right|$的出现次数(大小为 $n+1$),扫描整个链表,若第一次出现,则保留,并把计数数组更新;反之,将其从链表中删除.
...
时间复杂度为 $O(m)$,空间复杂度为 $O(n)$(以**空间换取时间**).

#### 2.3.25

>  **2019统考真题**：设线性表 $L=(a_1,a_2,a_3,\cdots,a_{n-2},a_{n-1},a_{n})$采用带头结点的单链表保存，链表中的结点定义如下:
>
> ```cpp
> typedef struct node{
>     int data;
...
> ```
>
> 请设计一个空间复杂度为 $O(1)$且时间上尽可能高效的算法，重新排列L中的各结点，得到线性表 $L'=(a_1,a_n,a_2,a_{n-1},a_3,a_{n-2})$。要求:
>
> 1)给出算法的基本设计思想。
>
> 2)根据设计思想，采用`C或C++语言`描述算法，关键之处给出注释。
...
利用**快慢指针**的思想(可参考**2.3.21**)找到中间结点,以此作为分届处,将后半部分进行逆置,然后再重新插入前半段中.

![](https://cdn.acwing.com/media/article/image/2023/07/22/85276_2502aea628-20230722220708.png) 

```cpp
void ChangeList(LinkList &L)
{
    NODE *p=L,*q=L;//p走一步,q走两步

    //由于p,q同步走,此时若q到达尾结点,说明p已到达中间节点
    ...
    NODE *p1=L->next;//指向第一个结点
    NODE *p2=p->next;//指向后一半逆置后的第一个结点

    p->next=NULL;//将前后断开

    while(p2!=NULL)//将后一半的结点插入前面中
    {
        NODE *r=p2->next;
